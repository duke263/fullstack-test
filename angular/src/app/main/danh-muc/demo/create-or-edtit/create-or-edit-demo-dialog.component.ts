...
export class CreateOrEditDemoDialogComponent extends AppComponentBase
    ...
    treeViewSelectValue: number;
    treeViewCheckboxValue: any[];
    treeViewSelectItems: TreeviewItem[];
    treeViewCheckboxItems: CheckboxTreeEditModel;
    treeViewCheckbox: FlatTreeSelectDto[];
    constructor(
        injector: Injector,
    ...
    ngOnInit() {
        ...
        forkJoin(
            ...
            this._lookupTableService.getAllDropDownTreeView(),
            this._lookupTableService.getAllMultipleCheckboxTreeView(),
        ).subscribe(([trangThaiHieuLuc, trangThaiDuyet, demo, treeViewSelect, treeViewCheckbox]) => {
            this.arrTrangHieuLuc = trangThaiHieuLuc;
            this.arrTrangThaiDuyet = trangThaiDuyet;
            this.demos = demo;
            this.treeViewSelectItems = this.getTreeviewItem(treeViewSelect);
            this.treeViewCheckbox = treeViewCheckbox;
            // Calendar
    ...
    save(): void {
        if (CommonComponent.getControlErr(this.form) === '') {
            ...
            this.http.post(URL, formdata).subscribe((res) => {
                ...
                this._demoService.createOrEdit(this.demoDto).subscribe((result) => {
                    if (result === 1) {
    ...
    private _setValueForEdit() {
        ...
        this.treeViewSelectValue = this.demoDto.dropDownSelectTree;

        this.treeViewCheckboxItems = {
            data: this.treeViewCheckbox,
            selected: this.demoDto.mutipleCheckboxTree?.split(',').map(e => Number(e)),
    ...
    private _getValueForSave() {
        ...
        // Combobox
        this.demoDto.dropDownSelectTree = this.treeViewSelectValue;
        this.demoDto.mutipleCheckboxTree = this.treeViewCheckboxValue.join();
